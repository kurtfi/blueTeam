...
async def test_ingest_and_run_simulation_e2e() -> None:
    ...
    scenario_path = os.path.join(
        current_dir, "../attack_simulator/scenarios/credential_dumping.yaml"
    )
    ...
    with patch(
        "attack_simulator.mcp_server.send_alert_to_webhook", 
        new_callable=AsyncMock
    ) as mock_send, patch(
        "attack_simulator.evaluator.playbook_match.check_actual_playbook", 
        new_callable=AsyncMock
    ) as mock_check, patch(
        "attack_simulator.evaluator.playbook_match.get_expected_playbooks", 
        new_callable=AsyncMock
    ) as mock_expected:
    ...
    async with pool.acquire() as conn:
        await conn.execute("DELETE FROM simulation_runs WHERE id = $1", uuid.UUID(run_id))
