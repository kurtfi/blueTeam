...
TriageCore provides a FastMCP-compliant server that exposes enterprise security operations tools to the Agentix orchestrator. It acts as the bridge between the AI agents and standard SOC platforms like Wazuh, Cortex, TheHive, and SOAR providers.
...
Start the TriageCore FastMCP server using `uv`:

```bash
cd src/TriageCore
uv sync
uv run python triage_core/main.py
```
