# SOCMCP: Technical Reference Manual
...
### FastMCP Integration
`SOCMCP` runs as a standalone Python process wrapping the [FastMCP](https://github.com/jlowin/fastmcp) framework. It exposes its endpoints via Server-Sent Events (SSE) on port `8081` (configurable via `FASTMCP_PORT`).
...
Security APIs often have rate limits or require complex authentication flows. The SOCMCP server utilizes structured logging (via `AgenticCommon`) to emit debug traces when tools fail to load or when upstream APIs timeout, preventing the main orchestrator from hanging.
...
`SOCMCP` requires privileged API keys to interact with the SOC ecosystem (Wazuh, Cortex, etc.). These credentials are injected purely through the environment (e.g., via `.env` or Kubernetes Secrets) and are **never** passed from the Agentix orchestrator. 
...
By running `SOCMCP` as a separate microservice, it can be placed within a strict network segment (e.g., inside the management VLAN) with firewalls allowing only inbound connections from the Agentix core, protecting the sensitive SOC APIs from direct internet exposure.
