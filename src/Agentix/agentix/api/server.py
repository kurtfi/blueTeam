    ...
    async def connect_mcp_background() -> None:
        ...
        while True:
            if getattr(app.state, "is_shutting_down", False):
                break
            try:
                logger.info("Connecting to SOC MCP Server...", url=settings.agentix_triage_core_url)
                
                async with sse_client(settings.agentix_triage_core_url) as soc_transport:
                    soc_read, soc_write = soc_transport
                    async with ClientSession(soc_read, soc_write) as session:
                        await session.initialize()
                        app.state.triage_core_session = session

                        # Sync TriageCore Tools into our Catalog
                        await app.state.catalog.register_mcp_client(session)
                        ...
                        try:
                            json_result = await session.call_tool("list_playbooks_json")
                            from agentix.tools.mcp_adapter import MCPToolAdapter
                ...
                logger.info("SOC MCP Server connection background task received cancellation. Exiting...")
                app.state.triage_core_session = None
                raise
            except Exception as e:
                app.state.triage_core_session = None
                logger.warning(
                    "Failed to connect or connection lost, retrying in 5 seconds...",
                    error=str(e),
                )
