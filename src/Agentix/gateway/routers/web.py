...
async def web_list_scenarios(current_user: dict[str, Any] = Depends(get_current_user)) -> list[dict[str, Any]]:
...
async def web_get_scenario_events(
    scenario_id: str, current_user: dict[str, Any] = Depends(get_current_user)
) -> list[dict[str, Any]]:
...
async def web_list_runs(
    limit: int = 20, offset: int = 0, current_user: dict[str, Any] = Depends(get_current_user)
) -> list[dict[str, Any]]:
...
async def web_run_results(run_id: str, current_user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:
...
async def web_sim_stats(current_user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:
...
async def web_get_llm_settings(current_user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:
...
async def web_trigger_bulk_run(
    payload: WebBulkRunRequest, current_user: dict[str, Any] = Depends(get_current_user)
) -> dict[str, Any]:
...
async def web_bulk_run_results(
    bulk_run_id: str, current_user: dict[str, Any] = Depends(get_current_user)
) -> dict[str, Any]:
