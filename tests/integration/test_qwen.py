...
pytestmark = pytest.mark.skipif(not os.getenv("OLLAMA_BASE_URL"), reason="OLLAMA_BASE_URL is not configured")
...
async def test_qwen_soc_analyst_flow():
    ...
    async with AsyncExitStack() as stack:
        ...
        matched_tools = await catalog.select(
            "T1003.008 alert", category_filter=config.tool_filters.categories, name_filter=config.tool_filters.names
        )
        ...
        if tool_calls:
            ...
            for tc in tool_calls:
                formatted_tool_calls.append(
                    {"id": tc.get("id", "call_12345"), "type": "function", "function": tc.get("function", {})}
                )

            messages.append(
                {"role": "assistant", "content": message.get("content", ""), "tool_calls": formatted_tool_calls}
            )

            messages.append(
                {"role": "tool", "tool_call_id": formatted_tool_calls[0]["id"], "content": "Playbook PB-001 found"}
            )
