...
def test_interpolate_string():
    ctx = PlaybookContext(
        alert={"agent_id": "007", "agent_name": "wazuh-agent-test", "src_ip": "10.0.0.99", "username": "attacker_user"}
    )
    step = PlaybookStep(
        order=0, title="Test ctx.agent_name", description="Isolate agent ctx.agent_id", group="Investigation"
    )
...
def test_render_instruction_with_deep_resolution():
    ...
    step = PlaybookStep(
        ...
        parameters={"username": "ctx.alert.username", "agent_id": "ctx.agent_id"},
