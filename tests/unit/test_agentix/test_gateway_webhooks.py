...
async def test_webhook_forwarding_success(mock_async_client):
    ...
    response = client.post("/v1/webhooks/siem", json=payload, headers=headers)
    ...
    assert "/v1/webhooks/siem" in call_args[0][0]
...
async def test_webhook_forwarding_failure(mock_async_client):
    ...
    response = client.post("/v1/webhooks/siem", json={"test": "data"}, headers={"X-Webhook-Signature": "signature"})
