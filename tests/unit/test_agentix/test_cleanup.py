...
async def test_cleanup_expired_workspaces(mock_now):
    with (
        patch("agentix.core.cleanup.SessionWorkspace.list_sessions") as mock_list,
        ...
        def mock_from_session_id_side_effect(sid):
            ...
            if sid == "sid_active_fresh":
                ...
                ws.get_metadata.return_value = {"created_at": created, "status": "cleaned", "cleaned_at": cleaned_time}
                return ws
...
async def test_run_periodic_cleanup():
    ...
    with (
        patch("agentix.core.cleanup.cleanup_expired_workspaces", new_callable=AsyncMock) as mock_clean,
