...
async def test_orchestrator_saves_draft_history_on_confirm():
        ...
        "tool_calls": [
            {
                "id": "call_123",
                "type": "function",
                "function": {"name": "isolate_endpoint", "arguments": '{"agent_id": "007"}'},
            }
        ],
    }
    ...
    mock_memory.set_metadata.assert_called_with("session_123", "draft_history", ANY)
...
async def test_orchestrator_resumes_from_draft_history():
    ...
    mock_memory.get_metadata.return_value = {"draft_history": draft_history}
    ...
    mock_tool.to_openai_schema = MagicMock(
        return_value={
            "type": "function",
            "function": {"name": "isolate_endpoint", "description": "isolate endpoint from network"},
        }
    ...
    mock_llm.chat.return_value = {"role": "assistant", "content": "Final Answer: Isolation is complete."}
